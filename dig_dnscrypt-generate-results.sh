...
EXPECTED_DOH_QUERIES=$(echo "($EXPERIMENT_DURATION_SECONDS * 1000) / $DELAY" | bc)
...
CONTAINER_ID=$(
	docker run \
		...
		-v "${DOMAIN_NAME_LIST_FILE}:/${DOMAIN_NAME_LIST_FILE}" \
		--ip "${CONTAINER_IP}" \
		--dns "${CONTAINER_IP}" \
...
docker exec \
	...
	-e SSLKEYLOGFILE="/${SSLKEYLOGFILE}" \
	"${CONTAINER_ID}" \
	bash -c "/dig.sh -m ${EXPECTED_DOH_QUERIES} -i ${DELAY} -p /${DOMAIN_NAME_LIST_FILE}"

./timer.sh $((EXPERIMENT_DURATION_SECONDS + 30))
